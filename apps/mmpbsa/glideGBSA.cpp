...
bool glideGBSA(std::string& dir, double& deltaG) {
...
// Ligand Calculation
    ...
    std::string keyword="i_m_Molecular_charge";
...
// Complex Calculation
    ...
    cmd="ambpdb -p LIG.prmtop < LIG.inpcrd > LIG_Tmp.pdb";
    std::cout <<cmd <<std::endl;
    system(cmd.c_str());    
    
    cmd="grep ATOM LIG_Tmp.pdb > LIG_strp.pdb";
    std::cout <<cmd <<std::endl;
    system(cmd.c_str());      
     
    cmd="cat ../../rec/m_rec_amber/Rec_min.pdb LIG_strp.pdb > Com.pdb";
    system(cmd.c_str());    
    ...
    std::string tleapFName="Com_leap.in";
    {
        ...
        tleapFile << "COM = loadpdb Com.pdb" << std::endl;
    ...
    sanderOut="Com_min_GB.out";
    cmd="sander  -O -i Com_min.in -o "+sanderOut+" -p Com.prmtop -c Com.inpcrd -ref Com.inpcrd  -x Com_.mdcrd -r Com_min.rst";
    std::cout <<cmd <<std::endl;
    system(cmd.c_str());  
    ...
    sanderOut="../../rec/m_rec_amber/Rec_minGB.out";
    double recGBen=0;
    success=pSanderOutput->getEnergy(sanderOut,recGBen);   
    
    deltaG=energy-recGBen-ligGBen;
...
void splitSDF(std::string& sdfFile, std::vector<std::string>& ligList){
    ...
    try {
        ...
        std::cout << "glideGBSA >> Cannot open file" << sdfFile << std::endl;
    } 
    ...
    while(inFile){
        ...
        if(fileLine.size()>=4 && fileLine.compare(0,4, delimter)==0){
            ...
            try {
                ...
                std::cout << "glideGBSA >> Cannot open file" << outputFName << std::endl;
            }
...
//    MPI_Barrier(MPI_COMM_WORLD);
    ...
    if (rank == 0) {        
        bool success=glideGBSAPO(argc, argv, podata);
        if(!success){
            error=1;           
        }        
    }
...
//                    if(!jobOut.error){
                    std::string deltaGStr=Sstrm<std::string,double>(jobOut.deltaG);
                    XMLElement * element = new XMLElement("error"); 
                    element->SetAttribute("ligand", jobOut.dirBuffer);
                    element->SetAttribute("deltaG", deltaGStr);
...
//            }
        ...
        while (1) {
            ...
            try{
                double deltaG=0;
                bool jobStatus=glideGBSA(dir, deltaG);   
                jobOut.deltaG=deltaG;
