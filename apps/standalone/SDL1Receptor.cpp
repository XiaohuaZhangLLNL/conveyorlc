...
#include "SDL1Receptor.h"
#include "SDL1ReceptorPO.h"
#include "InitEnv.h"
 ...
 * \verbatim
    ...
    SDL1Receptor  --input pdb.list --output out
...
void preReceptor(JobInputData& jobInput, JobOutData& jobOut, std::string& workDir, std::string& inputDir, std::string& dataPath){
    ...
    try{
        ...
        if(!fileExist(jobOut.pdbFilePath)){
            std::string mesg="SDL1Receptor::preReceptors: PDB file "+jobOut.pdbFilePath+" does NOT exist.";
            throw LBindException(mesg);   
        }
...
int main(int argc, char** argv) {
    ...
    bool success=SDL1ReceptorPO(argc, argv, podata);
    if(!success){
        exit(-1);
    }
    ...
    n["date"] = "Create By SDL1Receptor at " + timeStamp();
    relay::io::hdf5_append(n, recCdtFile);
    ...
    for(unsigned i=0; i<dirList.size(); ++i){
        ...
        std::cout << "Working on: " << dirList[i]->pdbFile  << std::endl;
